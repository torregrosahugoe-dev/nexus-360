...
// │  Versión 4.0 - MODO DIAGNÓSTICO AVANZADO                          │
// └───────────────────────────────────────────────────────────────────┘
...
// SECCIÓN 1: LÓGICA DEL MOTOR ASR (CON DIAGNÓSTICOS)
(function ensureGoogleCreds() {
  const b64 = process.env.GOOGLE_CREDENTIALS_B64;
  if (!b64) { console.warn('[DIAGNÓSTICO] GOOGLE_CREDENTIALS_B64 no configurado.'); return; }
  const credsPath = path.join(process.cwd(), 'gcp-stt.json');
  try {
    fs.writeFileSync(credsPath, Buffer.from(b64, 'base64'));
    process.env.GOOGLE_APPLICATION_CREDENTIALS = credsPath;
    console.log('[DIAGNÓSTICO] Credenciales de Google OK.');
  } catch (e) {
    console.error('[DIAGNÓSTICO] Error escribiendo credenciales:', e);
  }
})();

function createGoogleStream({ onData, onError, onEnd }) {
  console.log('[DIAGNÓSTICO] Creando stream de Google...');
  ...
  const recognizeStream = client.streamingRecognize(request)
    .on('error', (err) => {
        console.error('[DIAGNÓSTICO] Google Stream emitió un ERROR:', err);
        onError?.(err);
    })
    .on('data', (data) => {
        console.log('[DIAGNÓSTICO] Google Stream recibió DATOS:', JSON.stringify(data));
        ...
        if (result && result.alternatives && result.alternatives[0]) {
            ...
            console.log('[DIAGNÓSTICO] Google envió datos sin transcripción válida.');
        }
    })
    .on('end', () => {
        console.log('[DIAGNÓSTICO] Google Stream emitió "end".');
        onEnd?.();
    })
    .on('close', () => console.log('[DIAGNÓSTICO] Google Stream emitió "close".'));

  console.log('[DIAGNÓSTICO] Stream de Google creado y listeners añadidos.');
...
app.post('/listen', (req, res) => { const twiml = new VoiceResponse(); const connect = twiml.connect(); connect.stream({ url: `wss://${req.headers.host}/twilio-stream` }); twiml.pause({ length: 60 }); res.type('text/xml').send(twiml.toString()); });
...
// SECCIÓN 3: INICIALIZACIÓN Y CICLO DE CONVERSACIÓN (CON DIAGNÓSTICOS)
...
wss.on('connection', (ws, request) => {
    ...
    googleStream = createGoogleStream({ onData, onError: (e) => console.error('[DIAGNÓSTICO] onData tuvo un error:', e) });
    ...
    ws.on('message', (message) => {
        ...
        if (msg.event === 'start') {
            ...
            // Log para confirmar que el audio se está enviando
            console.log(`[DIAGNÓSTICO] Enviando chunk de audio a Google (${msg.media.payload.length} bytes)`);
