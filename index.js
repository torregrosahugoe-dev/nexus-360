// ┌───────────────────────────────────────────────────────────────────┐
// │  Nexus 300 - Servidor IVR Inteligente con ASR                     │
...
// SECCIÓN 2: LÓGICA DEL IVR (CON ENDPOINT /listen CORREGIDO)
...
// ENDPOINT /listen CORREGIDO CON PATRÓN DE ESPERA
app.post('/listen', (req, res) => {
  const twiml = new VoiceResponse();
  const connect = twiml.connect();
  connect.stream({ url: `wss://${req.headers.host}/twilio-stream` });

  // Después de que el stream termine, Twilio ejecutará estas instrucciones
  twiml.say({ voice: 'alice', language: 'es-MX' }, 'Procesando...');
  twiml.pause({ length: 20 }); // Mantenemos la llamada activa por 20 segundos
